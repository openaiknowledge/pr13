...
//        MGM return BERTVocabulary.lookupDictionary[token] ?? unkownTokenID
...
//        return vocabulary
        let wordsString = words.map { String($0) }
        
        let zipValues = zip(wordsString, values)

        let vocabulary = Dictionary(uniqueKeysWithValues: zipValues)
        return vocabulary
    }
}
